...
public class DeploymentArchiveProcessorUtils {
	...
	private static final boolean AUTH_SERVER_SSL_REQUIRED = Boolean
			.parseBoolean(System.getProperty("auth.server.ssl.required"));
	private static final boolean APP_SERVER_SSL_REQUIRED = Boolean
			.parseBoolean(System.getProperty("app.server.ssl.required"));
	...
	public static void modifyWebXMLForServletFilter(Archive<?> archive, TestClass testClass) {
		Document webXmlDoc;
		try {
			webXmlDoc = IOUtil.loadXML(archive.get(WEBXML_PATH).getAsset().openStream());
		} catch (Exception ex) {
			throw new RuntimeException("Error when processing " + archive.getName(), ex);
		}

		// We need to add filter declaration to web.xml
		log.info("Adding filter to " + testClass.getAnnotation(UseServletFilter.class).filterClass() + " with mapping "
				+ testClass.getAnnotation(UseServletFilter.class).filterPattern() + " for " + archive.getName());
		...
		// Limitation that all deployments of annotated class use same skipPattern.
		// Refactor if
		// something more flexible is needed (would require more tricky web.xml parsing
		// though...)
		...
		filterName.setTextContent(testClass.getAnnotation(UseServletFilter.class).filterName());
		urlPattern.setTextContent(IOUtil.getElementTextContent(webXmlDoc,
	...
	public static String getKeycloakResolverClass(Document doc) {
		try {
			...
			XPathExpression expr = xpath
					.compile("//web-app/context-param[param-name='keycloak.config.resolver']/param-value/text()");
	...
	public static void addFilterDependencies(Archive<?> archive, TestClass testClass) {
		...
		((WebArchive) archive).addAsLibraries(KeycloakDependenciesResolver
				.resolveDependencies((dependency + ":" + System.getProperty("project.version"))));

		Document jbossXmlDoc = IOUtil.loadXML(archive.get(JBOSS_DEPLOYMENT_XML_PATH).getAsset().openStream());
		IOUtil.removeNodeByAttributeValue(jbossXmlDoc, "dependencies", "module", "name",
	...
	public static void modifyOIDCAdapterConfig(Archive<?> archive, String adapterConfigPath) {
		try {
			AdapterConfig adapterConfig = IOUtil.loadJson(archive.get(adapterConfigPath).getAsset().openStream(),
					AdapterConfig.class);
			...
			if (AUTH_SERVER_SSL_REQUIRED) {
				...
				if (!truststore.exists()) {
					truststore = new File(DeploymentArchiveProcessorUtils.class
							.getResource("/keystore/keycloak.truststore").getFile());
				}

				((WebArchive) archive).addAsResource(truststore);

				log.debugf("Adding Truststore to the deployment, path %s, password %s, adapter path %s",
						truststore.getAbsolutePath(), TRUSTSTORE_PASSWORD, trustStorePathInDeployment);
			}
	...
	public static void modifySAMLAdapterConfig(Archive<?> archive, String adapterConfigPath) {
		...
		modifyDocElementAttribute(doc, "SingleSignOnService", "bindingUrl", AUTH_SERVER_REPLACED_URL,
				getAuthServerContextRoot());
		modifyDocElementAttribute(doc, "SingleLogoutService", "postBindingUrl", AUTH_SERVER_REPLACED_URL,
				getAuthServerContextRoot());
		modifyDocElementAttribute(doc, "SingleLogoutService", "redirectBindingUrl", AUTH_SERVER_REPLACED_URL,
				getAuthServerContextRoot());

		modifyDocElementAttribute(doc, "SingleSignOnService", "assertionConsumerServiceUrl", AUTH_SERVER_REPLACED_URL,
		...
		if (!truststore.exists()) {
			truststore = new File(
					DeploymentArchiveProcessorUtils.class.getResource("/keystore/keycloak.truststore").getFile());
		}
