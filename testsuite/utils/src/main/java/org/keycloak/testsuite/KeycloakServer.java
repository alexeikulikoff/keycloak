...
public class KeycloakServer {
	...
	public static KeycloakServer bootstrapKeycloakServer(String[] args) throws Throwable {
		...
		if (System.getProperties().containsKey("resources")) {
			...
			if (!System.getProperties().containsKey("keycloak.theme.dir")) {
				System.setProperty("keycloak.theme.dir",
						file(dir.getAbsolutePath(), "themes", "src", "main", "resources", "theme").getAbsolutePath());
			} else {
	...
	/**
	 * Detects the {@code jboss.server.data.dir} to use. If the System property
	 * {@code jboss.server.data.dir} is already set then the property value is used,
	...
	public void start() throws Throwable {
		...
		try {
			...
			// Note that the ResteasyServlet is configured via
			// server.undertowDeployment(...);
			// KEYCLOAK-14178
			...
			info("Started Keycloak !!!!  (http://" + config.getHost() + ":" + config.getPort() + "/auth"
					+ (config.getPortHttps() > 0
							? ", https://" + config.getHost() + ":" + config.getPortHttps() + "/auth"
							: "")
