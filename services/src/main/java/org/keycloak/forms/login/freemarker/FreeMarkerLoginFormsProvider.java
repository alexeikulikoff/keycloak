...
import org.keycloak.utils.DepartmentProducer;
...
public class FreeMarkerLoginFormsProvider implements LoginFormsProvider {
	...
	public Response createResponse(UserModel.RequiredAction action) {
		...
		case UPDATE_PASSWORD:
			boolean isRequestedByAdmin = user.getRequiredActions().stream().filter(Objects::nonNull)
	...
	protected Response createResponse(LoginFormsPages page) {
		...
		DepartmentProducer departments = new DepartmentProducer();
		...
		attributes.put("comrados", "Colorados");

		try {

			attributes.put("departments", departments.findAll());
		...
		case OAUTH_GRANT:
			...
			break;
		case CODE:
			attributes.put(OAuth2Constants.CODE,
					new CodeBean(accessCode, messageType == MessageType.ERROR ? getFirstMessageUnformatted() : null));
			break;
		case X509_CONFIRM:
	...
	protected Response processTemplate(Theme theme, String templateName, Locale locale) {
		try {
			...
			Response.ResponseBuilder builder = Response.status(status == null ? Response.Status.OK : status)
					.type(mediaType).language(locale).entity(result);
