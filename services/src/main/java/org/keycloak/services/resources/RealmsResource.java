...
public class RealmsResource {
	...
	public Object getProtocol(final @PathParam("realm") String name, final @PathParam("protocol") String protocol) {
		RealmModel realm = init(name);

		LoginProtocolFactory factory = (LoginProtocolFactory) session.getKeycloakSessionFactory()
				.getProviderFactory(LoginProtocol.class, protocol);
		if (factory == null) {
	 ...
	 * {@code clientId} in the given {@code realmName}.
	 * <p>
	 * This allows a client to refer to other clients just by their client id in
	 * URLs, will then redirect users to the actual client url. The client url is
	 * derived according to the rules of the base url in the client configuration.
	 * </p>
	 *
	 * @param realmName
	 * @param clientId
	...
	public Response getRedirect(final @PathParam("realm") String realmName,
		...
		if (client.getRootUrl() != null && (client.getBaseUrl() == null || client.getBaseUrl().isEmpty())) {
			...
			targetUri = KeycloakUriBuilder
					.fromUri(ResolveRelative.resolveRelativeUri(session, client.getRootUrl(), client.getBaseUrl()))
					.build();
		}
	 ...
	 * {@link org.keycloak.services.resource.RealmResourceSPI} to resolve
	...
	public Object resolveRealmExtension(@PathParam("realm") String realmName,
