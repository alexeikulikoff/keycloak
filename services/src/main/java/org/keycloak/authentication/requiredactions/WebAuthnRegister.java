...
public class WebAuthnRegister implements RequiredActionProvider, CredentialRegistrator {
	...
	public void requiredActionChallenge(RequiredActionContext context) {
		...
		// Otherwise the platform's default charset is used and it might cause problems
		// later when
		...
		if (avoidSameAuthenticatorRegister) {
			List<CredentialModel> webAuthnCredentials = session.userCredentialManager()
					.getStoredCredentialsByType(context.getRealm(), userModel, getCredentialType());
		...
		String isSetRetry = context.getHttpRequest().getDecodedFormParameters()
				.getFirst(WebAuthnConstants.IS_SET_RETRY);

		Response form = context.form().setAttribute(WebAuthnConstants.CHALLENGE, challengeValue)
				.setAttribute(WebAuthnConstants.USER_ID, userId).setAttribute(WebAuthnConstants.USER_NAME, username)
				...
				.setAttribute(WebAuthnConstants.IS_SET_RETRY, isSetRetry).createForm("webauthn-register.ftl");
	...
	public void processAction(RequiredActionContext context) {
		...
		// check User Verification by considering a malicious user might modify the
		// result of calling WebAuthn API
		...
		try {
			...
			context.getEvent().detail(WebAuthnConstants.PUBKEY_CRED_ID_ATTR, publicKeyCredentialId)
	...
	private WebAuthnRegistrationManager createWebAuthnRegistrationManager() {
		return new WebAuthnRegistrationManager(
				Arrays.asList(new NoneAttestationStatementValidator(), new PackedAttestationStatementValidator(),
						new TPMAttestationStatementValidator(), new AndroidKeyAttestationStatementValidator(),
						new AndroidSafetyNetAttestationStatementValidator(),
						new FIDOU2FAttestationStatementValidator()),
				this.certPathtrustValidator, new DefaultSelfAttestationTrustworthinessValidator(),
	...
	private String stringifyExcludeCredentialIds(List<String> credentialIdsList) {
		if (credentialIdsList == null || credentialIdsList.isEmpty())
			return "";
		StringBuilder sb = new StringBuilder();
	...
	private void setErrorResponse(RequiredActionContext context, final String errorCase, final String errorMessage) {
		...
		case WEBAUTHN_ERROR_REGISTER_VERIFICATION:
			...
			context.getEvent().detail(ERR_LABEL, errorCase).detail(ERR_DETAIL_LABEL, errorMessage)
					.error(Errors.INVALID_USER_CREDENTIALS);
			errorResponse = context.form().setError(errorCase)
					.setAttribute(WEB_AUTHN_TITLE_ATTR, WEBAUTHN_REGISTER_TITLE).createWebAuthnErrorPage();
